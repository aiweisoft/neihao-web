    ...
    <view class="page-card">
      <view class="page-header">
        <view class="page-header-left">
          ...
          <view class="page-title">编辑保养计划</view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    getDetail(id) {
      uni.showLoading({ mask: true })
      db.collection(dbCollectionName).doc(id).field("device_id,plan_name,maintenance_type,plan_date,cycle_type,cycle_value,next_date,remind_before_days,description,status,completed_date,remark").get().then((res) => {
        const data = res.result.data[0]
        if (data) {
          this.formData = Object.assign(this.formData, data)
          this.formData.cycle_value = data.cycle_value || null
          this.formData.remind_before_days = data.remind_before_days || 7
