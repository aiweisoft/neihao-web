    ...
    <view class="page-card">
      <view class="page-header">
        <view class="page-header-left">
          ...
          <view class="page-title">维修记录</view>
        </view>
      ...
      <view class="page-body">
        <unicloud-db ref="udb" collection="medical-device-repair" :where="where" page-data="replace" :orderby="orderby"
          ...
          <uni-table ref="table" :loading="loading" border stripe type="selection"
            ...
            <uni-tr>
              ...
              <uni-th align="center">维修日期</uni-th>
              <uni-th align="center">维修费用</uni-th>
              <uni-th align="center">维修结果</uni-th>
              <uni-th align="center">操作</uni-th>
            </uni-tr>
            <uni-tr v-for="(item, index) in tableData" :key="index">
              <uni-td align="center">{{ item.device_id_text || '-' }}</uni-td>
              <uni-td align="center">{{ item.requester_text || '-' }}</uni-td>
              ...
              <uni-td align="center">{{ item.repair_date_text || '-' }}</uni-td>
              <uni-td align="center">{{ item.cost != null ? '¥' + item.cost.toFixed(2) : '-' }}</uni-td>
              <uni-td align="center">
                <uni-tag :type="resultTag(item.result)" :text="resultText(item.result)" size="small" />
              </uni-td>
              <uni-td align="center">
...
export default {
  ...
  methods: {
    async onqueryload(data) {
      ...
      const requestIds = [...new Set(data.map(i => i.repair_request_id).filter(Boolean))]
      const [deviceRes, requestRes] = await Promise.all([
        ...
        requestIds.length ? db.collection('medical-device-repair-request').where({ _id: dbCmd.in(requestIds) }).get() : Promise.resolve({ result: { data: [] } })
      ])
      const deviceMap = {}
      deviceRes.result.data.forEach(d => { deviceMap[d._id] = d.name })
      const requestMap = {}
      requestRes.result.data.forEach(d => { requestMap[d._id] = d })
      this.tableData = data.map(item => {
        const req = requestMap[item.repair_request_id] || {}
        return {
          ...item,
          device_id_text: deviceMap[item.device_id] || '-',
          requester_text: req.requester || '-',
          request_date_text: req.request_date ? new Date(req.request_date).toLocaleDateString('zh-CN') : '-',
          urgency: req.urgency,
          repair_date_text: item.repair_date ? new Date(item.repair_date).toLocaleDateString('zh-CN') : '-'
        }
      })
    },
    loadData(clear = true) {
    ...
    resultText(val) {
      return { 1: '已修复', 2: '部分修复', 3: '无法修复' }[val] || '-'
    },
    resultTag(val) {
      return { 1: 'success', 2: 'warning', 3: 'error' }[val] || 'default'
    },
    delTable() {
